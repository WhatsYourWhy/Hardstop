...
Sentinel is intentionally local-first, but it plays well with the rest of your product stack. Use this guide to connect Sentinel runs to collaboration, code, and automation tools so alerts stay in sync with how your team works.
...
- **Recommended flow:**
  ...
  2) Run `sentinel brief --today --format json > /tmp/brief.json` after each pipeline run.
...
Automate Sentinel alongside your CI so code changes and risk signals stay paired.

- **Run on a schedule:** Trigger `sentinel run --since 24h` in a scheduled workflow to fetch and ingest events.
- **Attach artifacts:** Save `sentinel brief --today --format md` as a build artifact for reviewers.
...
If you use AI agents or automation runners, treat Sentinel as the deterministic layer and let the agent handle outreach.

- Use the brief JSON output as the agent's context window and have it decide who to notify.
- Allow agents to call `sentinel sources test <id>` for diagnostics, but gate database writes behind human approval.
- Cache agent prompts with the Sentinel version and config commit so responses are reproducible.

## Integration directory

Sentinel itself avoids SaaS lock-in, so choose the right connector for your stack:

- **Notifications:** Slack, Microsoft Teams, email (SMTP).
- **Work management:** Linear, Jira, Asana — create issues from high-impact alerts.
- **Data sync:** Airflow/Prefect or GitHub Actions to orchestrate `sentinel run` and `sentinel brief` on a schedule.
...
2) Map Sentinel classifications to Linear priority (e.g., class 2 → P1, class 1 → P2).
3) Script a small bridge:
   - Query `sentinel brief --today --format json`.
...
4) Run the bridge after each successful `sentinel run`.

## Quick checklist

- [ ] Decide where alerts live (Slack channel, Linear team, GitHub PR comment, etc.).
- [ ] Schedule `sentinel run` + `sentinel brief` at the cadence your team needs.
