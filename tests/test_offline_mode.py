...
def test_offline_mode_with_file_fixtures(tmp_path, session):
    ...
    # Monkeypatch requests.get to raise if any network call is attempted
    def network_tripwire(*args, **kwargs):
        raise RuntimeError(
            "Network call detected in offline mode test. "
            "This path should not invoke any HTTP adapters. "
            f"Attempted call: {args}, {kwargs}"
        )
    
    with patch('requests.get', side_effect=network_tripwire):
        with patch('requests.post', side_effect=network_tripwire):
            with patch('requests.request', side_effect=network_tripwire):
                ...
                # If we get here without raising, no network calls were made
                assert True, "Offline mode test passed - no network adapters invoked"
